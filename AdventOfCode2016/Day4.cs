  ...
  class Day4
  {
    static void Main4(string[] args)
    {
      ...
      foreach (var room in rooms)
      {
        ...
        var sortedLetters = letters.OrderByDescending(l => freqs[l - 'a']); // Needs to be a stable sort
    ...
    private static readonly Regex Matcher = new Regex(@"(?<name>.*)-(?<sector>\d+)\[(?<checksum>\w\w\w\w\w)\]", RegexOptions.Compiled);

    private const string Input = @"aczupnetwp-mfyyj-opalcexpye-977[peyac]
