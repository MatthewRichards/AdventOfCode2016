  ...
  internal class Day10
  {
    ...
    private class Bot : Destination
    {
      ...
      public void Give(int value)
      {
        if (Values.Count >= 2)
        {
          throw new InvalidOperationException("Bot already has two values!");
        }
      ...
      public bool Act()
      {
        if (Values.Count == 2)
        {
          ...
          LowDestination.Give(min);
          HighDestination.Give(max);
    ...
    private class Factory
    {
      ...
      {
        if (!Bots.ContainsKey(id)) Bots.Add(id, new Bot());
        return Bots[id];
      }

      public Output GetOutput(int id)
      {
        if (!Outputs.ContainsKey(id)) Outputs.Add(id, new Output());
        return Outputs[id];
      }

      public void AddInstruction(Match parsedInstruction)
      {
        if (parsedInstruction.Groups["value"].Success)
        {
          GetBot(int.Parse(parsedInstruction.Groups["goes_to"].Value)).Give(int.Parse(parsedInstruction.Groups["value"].Value));
        }
        else
        {
          var bot = GetBot(int.Parse(parsedInstruction.Groups["bot_gives"].Value));
          var lowId = int.Parse(parsedInstruction.Groups["give_low_id"].Value);
          var highId = int.Parse(parsedInstruction.Groups["give_high_id"].Value);
          bot.LowDestination = parsedInstruction.Groups["give_low_type"].Value == "bot" ? (Destination)GetBot(lowId) : GetOutput(lowId);
          bot.HighDestination = parsedInstruction.Groups["give_high_type"].Value == "bot" ? (Destination)GetBot(highId) : GetOutput(highId);
        }
      }

      public bool ActAllBots()
      {
        return Bots.Values.Aggregate(false, (acc, bot) => acc | bot.Act());
      }
    }

    private static readonly Regex Parser = new Regex(
      @"value (?<value>\d+) goes to bot (?<goes_to>\d+)$|bot (?<bot_gives>\d+) gives low to (?<give_low_type>bot|output) (?<give_low_id>\d+) and high to (?<give_high_type>bot|output) (?<give_high_id>\d+)$");
    
    private const string Input = @"bot 49 gives low to bot 118 and high to bot 182
